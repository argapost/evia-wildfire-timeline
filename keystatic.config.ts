  ...
  collections: {
    events: collection({
      ...
      schema: {
        ...
        end: fields.text({ label: 'End Date', description: 'Optional; leave empty for point events.' }),
        datePrecision: fields.select({
          label: 'Date Precision',
          options: datePrecisionValues.map((value) => ({ label: value, value })),
          defaultValue: 'day'
        }),
        ...
        actors: fields.array(fields.text({ label: 'Actor ID', validation: { isRequired: true } }), {
          label: 'Actors',
          itemLabel: (props) => props.value || 'Actor'
        }),
        places: fields.array(fields.text({ label: 'Place ID', validation: { isRequired: true } }), {
          label: 'Places',
        ...
        sourceRefs: fields.array(fields.text({ label: 'Source Ref ID', validation: { isRequired: true } }), {
        ...
        coverImage: fields.text({ label: 'Cover Image Ref ID' }),
        ...
        content: fields.markdoc({ label: 'Event Commentary', extension: 'md' })
      }
    }),
    actors: collection({
      label: 'Actors',
